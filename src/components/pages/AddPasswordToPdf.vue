<template>
  <div class="add-password-page">
    ...
    <h1>🔒 Adicionar Senha a PDF</h1>
    <p class="subtitle">Proteja seus arquivos PDF com uma senha de forma rápida e segura!</p>
    ...
    <div class="password-options">
      ...
      <div class="form-group">
        <label for="password">Senha para o PDF:</label>
        <input 
          id="password" 
          v-model="password" 
          type="password" 
      ...
      <p class="info-text">Sua senha será usada para proteger o PDF. Escolha uma senha forte para maior segurança.</p>
    </div>
    ...
    <button @click="addPassword" :disabled="!file || !password || password !== confirmPassword || processing" class="primary-btn">
...
const addPassword = async () => {
  ...
  if (!password.value) {
    error.value = 'Por favor, digite uma senha para o PDF.';
    return;
  }
  
  if (password.value !== confirmPassword.value) {
    error.value = 'As senhas não coincidem. Por favor, verifique.';
    return;
  }
  ...
  try {
    const response = await axios.post('/api/add-pdf-password', formData, {
      responseType: 'blob',
